if config["remove_common_variants"] and len(SAMPLES) > 1:
    ...
    rule prepare4bcftools:
        ...
        log:
            stdout = os.path.join(outdir, "{vartype}/{tool}/logs/{sample}_indexing.stdout"),
            stderr = os.path.join(outdir, "{vartype}/{tool}/logs/{sample}_indexing.stderr")
        conda:
            "../envs/bcftools.yml"
        shell:
    ...
    rule bcftools:
        ...
        input:
            compressed = expand(os.path.join(outdir, "{{vartype}}/{{tool}}/{sample}.vcf.gz"), sample = SAMPLES),
            indexed = expand(os.path.join(outdir, "{{vartype}}/{{tool}}/{sample}.vcf.gz.tbi"), sample = SAMPLES)
        output:
