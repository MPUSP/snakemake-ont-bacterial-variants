...
## Filtering of variants by genomic region(s)

```{r masked, echo=FALSE, warning=FALSE, results = 'asis'}
if (file.exists(snakemake@params[["maskedregions"]])) {
  masked <- read.csv(as.character(snakemake@params[["maskedregions"]]), header = FALSE, sep = "\t", comment.char = "#", col.names = c("contig", "start", "stop"))
  if (nrow(masked) > 0) {
    ...
    cat("The following regions were filtered, possible variants in these regions are not reported below:")
    DT::datatable(
      masked,
...
if (nrow(clair) > 0) {
  ...
  clair <- clair[,c(1,2,8,3,4,5,6,7)] %>%
    # Fix issue of T being interpreted as TRUE if no other nucleotide in the reference or alternative allele column
