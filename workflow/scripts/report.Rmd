...
Reads were filtered using Filtlong (see [here](https://github.com/rrwick/Filtlong)) to enrich for long high-quality reads:
```
...
Clair3 is a deep learning-based variant caller (see [here](https://doi.org/10.1038/s43588-022-00387-x) and [here](https://github.com/HKU-BAL/Clair3)). Clair3 (v1.0.4) was run on the alignment files produced by NGMLR using standard parameters:
```
...
clair <- read.table(as.character(snakemake@input[["clair3"]]), sep = "\t", header = F, col.names = c("contig", "position", "ID", "ref.allele", "alt.allele", "quality", "filter", "info", "format", "sample"))
clair$quality <- as.numeric(as.character(clair$quality))
...
medaka <- read.table(as.character(snakemake@input[["medaka"]]), sep = "\t", header = F, col.names = c("contig", "position", "ID", "ref.allele", "alt.allele", "quality", "filter", "info", "format", "sample"))
medaka$quality <- as.numeric(as.character(medaka$quality))
...
cuteSV (v2.0.3) was run on NGMLR-aligned reads using standard settings suggested for ONT data:
...
cutesv <- read.table(as.character(snakemake@input[["cutesv"]]), sep = "\t", header = F, col.names = c("contig", "position", "ID", "reference", "alternative", "quality", "filter", "info", "format", "sample"))
cutesv$position <- as.numeric(as.character(cutesv$position))
cutesv$quality <- as.numeric(as.character(cutesv$quality))
# Determine SV type
...
Sniffles2 is a structural variant caller designed for long-read sequencing data aligned with the long-read ampper NGMLR (see [here](https://doi.org/10.1038/s41592-018-0001-7) for the original Sniffles / NGMLR publication,
[here](https://doi.org/10.1101/2022.04.04.487055) for the Sniffles2 update, and [here](https://github.com/fritzsedlazeck/Sniffles) for more information).
...
sniffles <- read.table(as.character(snakemake@input[["sniffles"]]), sep = "\t", header = F, col.names = c("contig", "position", "ID", "reference", "alternative", "quality", "filter", "info", "format", "sample"))
sniffles$position <- as.numeric(as.character(sniffles$position))
sniffles$quality <- as.numeric(as.character(sniffles$quality))
# Determine SV type
